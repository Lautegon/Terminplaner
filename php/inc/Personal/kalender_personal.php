...
function getCalender($date,$headline = array('Mo','Di','Mi','Do','Fr','Sa','So')) {
    ...
    for( $i = 1; $i <= $sum_days; $i++ ) {
		...
		if(in_array($aktuell,$feiertag) == true) {
			 echo "<div class=\"day feiertag\"><a href=\"Personal.php?timestamp=".$date."&tagesid=".date("ym",$date).sprintf("%02d",$i)."\"class=\"taglink\">".sprintf("%02d",$i)."</a></div>\n";
			 continue;
		}
        ...
        if( $i == date('d',$date) && date('m.Y',$date) == date('m.Y')&& $day_name == 'Sun') {
            echo "<div class=\"day current sun\"><a href=\"Personal.php?timestamp=".$date."&tagesid=".date("ym",$date).sprintf("%02d",$i)."\"class=\"taglink\">".sprintf("%02d",$i)."</a></div>\n";
			continue;
        }
	
		//wenn nicht Aktueller Tag Sonntag --> Rot einfärben
		if($day_name == 'Sun') {
			echo "<div class=\"day sun\"><a href=\"Personal.php?timestamp=".$date."&tagesid=".date("ym",$date).sprintf("%02d",$i)."\"class=\"taglink\">".sprintf("%02d",$i)."</a></div>\n";
			continue;
		}
        ...
        if( $i == date('d',$date) && date('m.Y',$date) == date('m.Y')) {
            echo "<div class=\"day current\"><a href=\"Personal.php?timestamp=".$date."&tagesid=".date("ym",$date).sprintf("%02d",$i)."\"class=\"taglink\">".sprintf("%02d",$i)."</a></div>\n";
			continue;
        }
		...
		//Normale Tage Ausgeben
            ...
            echo "<div class=\"day normal\"><a href=\"Personal.php?timestamp=".$date."&tagesid=".date("ym",$date).sprintf("%02d",$i)."\"class=\"taglink\">".sprintf("%02d",$i)."</a></div>\n";
        }
